      ...
      try {
        ...
        switch (parsed.name) {
          ...
          case 'TokensBought': {
            ...
            // Insert trade record for price chart
            if (tokenData) {
              await supabaseAdmin.from('token_trades').insert({
                ...
                trade_type:     'buy',
                eth_amount:     ethInNum,
                token_amount:   parseFloat(ethers.formatUnits(tokensOut, 18)),
                price_eth:      newPriceNum,
          ...
          case 'TokensSold': {
            ...
            const { data: tokenDataSell } = await supabaseAdmin
              .from('tokens')
              .select('id')
              .eq('contract_address', token.toLowerCase())
              .single()
            ...
            if (tokenDataSell) {
              await supabaseAdmin.from('token_trades').insert({
                token_id:       tokenDataSell.id,
                wallet_address: seller.toLowerCase(),
                trade_type:     'sell',
                eth_amount:     ethOutNum,
                token_amount:   parseFloat(ethers.formatUnits(tokensIn, 18)),
                price_eth:      newPriceNum,
                volume_usd:     volumeUsd,
                tx_hash:        log.transactionHash,
              })
