...
export async function GET(req: NextRequest) {
  ...
  let query = supabaseAdmin.from('kol_calls').select('*,launchers(twitter_handle,badge),tokens(ticker,name)').order('called_at', { ascending: false }).limit(50)
...
export async function POST(req: NextRequest) {
  ...
  const { data: existing } = await supabaseAdmin.from('kol_calls').select('id').eq('launcher_id', launcher.id).eq('token_id', token_id).single()
  if (existing) return NextResponse.json({ error: 'Already called this token' }, { status: 400 })
  const { data: call, error } = await supabaseAdmin.from('kol_calls').insert({
    launcher_id: launcher.id, token_id, thesis: thesis || null,
    price_at_call: token.price_eth, mktcap_at_call: token.market_cap_usd,
    accuracy_status: 'pending', called_at: new Date().toISOString(),
  }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
