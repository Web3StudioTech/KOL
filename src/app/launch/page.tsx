        ...
        <div style={{ textAlign:'center', padding:'2rem', maxWidth:'520px' }}>
          ...
          <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'4px', padding:'16px', marginBottom:'1.5rem', textAlign:'left' }}>
            {[
              ...
              ['Launch fee paid', '0.0004 ETH'],
              ['Your royalty', '0.70% per trade'],
            ].map(([k, v]) => (
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'24px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'4px', padding:'24px' }}>
                <h3 style={{ fontFamily:'Bebas Neue,sans-serif', fontSize:'24px', marginBottom:'20px', borderBottom:'1px solid var(--border)', paddingBottom:'12px', letterSpacing:'1px' }}>Token Details</h3>
              ...
              {/* Media */}
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'4px', padding:'24px' }}>
                <h3 style={{ fontFamily:'Bebas Neue,sans-serif', fontSize:'24px', marginBottom:'8px', borderBottom:'1px solid var(--border)', paddingBottom:'12px', letterSpacing:'1px' }}>Media</h3>
                <div style={{ padding:'10px 12px', background:'rgba(0,229,255,0.05)', border:'1px solid rgba(0,229,255,0.15)', borderRadius:'3px', marginBottom:'16px', fontSize:'12px', color:'var(--muted)', lineHeight:1.6 }}>
                  💡 Use square images for token logo and horizontal for banner. Compatible with DexScreener, DexTools and CoinGecko.
                </div>
                <ImageUpload
                  type="token"
                  wallet={address || ''}
                ...
                <ImageUpload
                  ...
                  label="Banner Image (Horizontal 3:1)"
                />
              </div>
              ...
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'4px', padding:'24px' }}>
                <h3 style={{ fontFamily:'Bebas Neue,sans-serif', fontSize:'24px', marginBottom:'20px', borderBottom:'1px solid var(--border)', paddingBottom:'12px', letterSpacing:'1px' }}>
                  Social Links <span style={{ fontSize:'14px', color:'var(--muted)', fontFamily:'Barlow,sans-serif', fontWeight:400 }}>(all optional)</span>
                </h3>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'4px', padding:'24px' }}>
                ...
                <div style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:'3px', padding:'14px', marginBottom:'16px' }}>
                  ...
                  {[
                    ['Launch fee',    '0.0004 ETH',         'var(--text)'],
                    ['Trading fee',   '1% per trade',        'var(--text)'],
                    ['Your royalty',  '0.70% per trade',     'var(--green)'],
                ...
                {!connected ? (
                  ...
                  >
                    {loading ? 'Launching...' : '⚡ Launch — 0.0004 ETH'}
                  </button>
                )}
              ...
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'4px', padding:'24px' }}>
                ...
                <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'4px', padding:'16px' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'12px' }}>
                    <div style={{ width:44, height:44, borderRadius:'6px', background:imageUrl?`url(${imageUrl}) center/cover`:'var(--bg3)', border:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'Bebas Neue,sans-serif', fontSize:'13px', color:'var(--accent)', flexShrink:0 }}>
                      {!imageUrl && (ticker.slice(0,3)||'???')}
                    </div>
                    <div>
                      <div style={{ fontFamily:'Bebas Neue,sans-serif', fontSize:'22px', letterSpacing:'1px', lineHeight:1 }}>${ticker||'TICKER'}</div>
                  ...
                  {tagline && <p style={{ fontSize:'13px', color:'var(--muted)', lineHeight:1.5, marginBottom:'10px' }}>{tagline}</p>}
              ...
              {/* Badges */}
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'4px', padding:'24px' }}>
                <h3 style={{ fontFamily:'Bebas Neue,sans-serif', fontSize:'24px', marginBottom:'4px', letterSpacing:'1px' }}>Get Badges</h3>
                <p style={{ fontSize:'12px', color:'var(--muted)', marginBottom:'16px' }}>All badges are FCFS — first come, first served. Limited supply.</p>
                <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                  {[
                    { img:'/badges/anon-badge.png',      label:'Anon Badge',      color:'#7c3aed', req:'$10K trading volume',          limit:'5,000' },
                    { img:'/badges/trader-badge.png',    label:'Trader Badge',    color:'#06b6d4', req:'$50K trading volume',          limit:'2,500' },
                    { img:'/badges/kol-badge.png',       label:'KOL Badge',       color:'#3b82f6', req:'2,000+ Twitter followers',     limit:'700' },
                    { img:'/badges/creator-badge.png',   label:'Creator Badge',   color:'#f59e0b', req:'Project hits $10M market cap', limit:'1,000' },
                    { img:'/badges/builder-badge.png',   label:'Builder Badge',   color:'#10b981', req:'Project hits $50M market cap', limit:'500' },
                    { img:'/badges/kol-crown-badge.png', label:'KOL Crown Badge', color:'#ec4899', req:'5,000+ Twitter followers',     limit:'300' },
                  ].map(b => (
                    <div key={b.label} style={{ padding:'10px 14px', background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:'3px', display:'flex', alignItems:'center', gap:'10px' }}>
                      <img src={b.img} alt={b.label} style={{ width:36, height:36, objectFit:'contain', background:'#080b0f', borderRadius:'6px', flexShrink:0 }} />
                      <div style={{ flex:1 }}>
                        <div style={{ fontFamily:'Barlow Condensed,sans-serif', fontSize:'13px', fontWeight:700, letterSpacing:'1px', color:b.color, marginBottom:'2px' }}>{b.label}</div>
                        <div style={{ fontSize:'11px', color:'var(--muted)' }}>{b.req}</div>
                      </div>
                      <div style={{ fontFamily:'Barlow Condensed,sans-serif', fontSize:'11px', fontWeight:700, letterSpacing:'1px', color:'var(--muted)', textAlign:'right', flexShrink:0 }}>
                        Limit<br/>{b.limit}
                      </div>
                    </div>
                  ))}
