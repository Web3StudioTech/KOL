      ...
      <div style={{maxWidth:'1400px',margin:'0 auto',padding:'32px 40px'}}>
        {tab==='overview' && (
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px'}}>
            {[['Total Launchers',total,'var(--accent)'],['KOL Badge',launchers.filter(l=>l.badge==='kol'||l.badge==='kol_crown').length,'#3b82f6'],['Trader Badge',launchers.filter(l=>l.badge==='trader'||l.badge==='anon').length,'#06b6d4'],['Creator/Builder',launchers.filter(l=>l.badge==='creator'||l.badge==='builder').length,'#f59e0b'],['Banned',launchers.filter(l=>l.is_banned).length,'var(--accent2)']].map(([label,value,color]) => (
                  ...
                  <tr key={l.id} style={{borderBottom:'1px solid var(--border)',background:l.is_banned?'rgba(255,61,107,0.04)':'transparent'}}>
                    ...
                    <td style={{padding:'10px 14px'}}>
                      <div style={{display:'flex',gap:'4px'}}>
                        ...
                        {(l.follower_count>=2000 && l.badge!=='kol' && l.badge!=='kol_crown') && (
